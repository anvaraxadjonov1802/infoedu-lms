...
export const Header: React.FC<HeaderProps> = ({ setMobileOpen }) => {
  ...
  const getPageTitle = (page: PageView): string => {
    switch (page) {
      case 'dashboard':
        return 'Bosh sahifa';
      case 'courses':
        return 'Mening kurslarim';
      case 'course_detail':
        return 'Kurs haqida ma’lumot';
      case 'theory':
        return 'Nazariy darslar';
      case 'presentations':
        return 'Taqdimotlar';
      case 'videos':
        return 'Video darslar';
      case 'tests':
        return 'Testlar katalogi';
      case 'test_taking':
        return 'Test topshirish';
      case 'test_result':
        return 'Test natijasi';
      case 'results':
        return 'Natijalarim';
      case 'progress':
        return 'O‘qish jarayoni';
      case 'notifications':
  ...
  const filteredTests = globalSearchQuery.trim()
    ? (Object.values(tests) as Test[]).filter(
        (t) =>
          t.title.toLowerCase().includes(globalSearchQuery.toLowerCase()) ||
          t.courseName.toLowerCase().includes(globalSearchQuery.toLowerCase())
      )
    : [];
    ...
    <header className="sticky top-0 z-30 h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 lg:px-8 flex items-center justify-between gap-4">
          ...
          <div className="absolute left-0 right-0 top-12 bg-white rounded-2xl shadow-xl border border-slate-200 p-3 max-h-80 overflow-y-auto z-50">
            ...
            {filteredCourses.length === 0 && filteredTests.length === 0 ? (
              <p className="text-xs text-slate-500 py-4 text-center">
                “{globalSearchQuery}” bo‘yicha hech narsa topilmadi.
              </p>
            ) : (
                  ...
                  <div>
                    ...
                    {filteredCourses.map((c) => (
                      <button
                        key={c.id}
                        onClick={() => {
                          navigateTo('course_detail', { courseId: c.id });
                          setSearchOpen(false);
                        }}
                  ...
                  <div>
                    ...
                    {filteredTests.map((t) => (
                      <button
                        key={t.id}
                        onClick={() => {
                          navigateTo('test_taking', { testId: t.id });
                          setSearchOpen(false);
                        }}
                        className="w-full flex items-center gap-2 p-2 rounded-xl hover:bg-slate-50 text-left text-xs text-slate-800"
                      >
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className={`relative ${import.meta.env.VITE_DEMO_MODE === 'true' ? '' : 'hidden'}`}>
          ...
          {roleMenuOpen && (
            <div className="absolute right-0 top-10 w-44 bg-white rounded-xl shadow-xl border border-slate-200 py-1 z-50 animate-in fade-in zoom-in-95">
              <div className="px-3 py-1.5 text-[11px] font-medium text-slate-400 uppercase tracking-wider border-b border-slate-100">
                Rolni Tanlang
              </div>
              <button
                onClick={() => {
                ...
                className={`w-full text-left px-3 py-2 text-xs font-medium hover:bg-slate-50 flex items-center justify-between ${
                  user.role === 'student' ? 'text-blue-600 font-bold bg-blue-50/50' : 'text-slate-700'
                }`}
              >
                Talaba rejimi
                {user.role === 'student' && <CheckCheck className="w-3.5 h-3.5 text-blue-600" />}
              </button>
              <button
                onClick={() => {
                  switchRole('teacher');
                ...
                className={`w-full text-left px-3 py-2 text-xs font-medium hover:bg-slate-50 flex items-center justify-between ${
                  user.role === 'teacher' ? 'text-purple-600 font-bold bg-purple-50/50' : 'text-slate-700'
                }`}
              >
                O‘qituvchi rejimi
                {user.role === 'teacher' && <CheckCheck className="w-3.5 h-3.5 text-purple-600" />}
              </button>
              <button
                onClick={() => {
                  switchRole('admin');
                ...
                className={`w-full text-left px-3 py-2 text-xs font-medium hover:bg-slate-50 flex items-center justify-between ${
                  user.role === 'admin' ? 'text-rose-600 font-bold bg-rose-50/50' : 'text-slate-700'
                }`}
              >
                Admin rejimi
                {user.role === 'admin' && <CheckCheck className="w-3.5 h-3.5 text-rose-600" />}
              </button>
            </div>
          )}
        </div>
        ...
        <button
          ...
          title={theme === 'light' ? 'Qorong‘u rejim (preview)' : 'Yorug‘ rejim'}
        >
                  ...
                  notifications.slice(0, 5).map((n) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        if (n.linkTarget) {
                          if (n.linkTarget.startsWith('course-')) {
                            navigateTo('course_detail', { courseId: n.linkTarget });
                          } else if (n.linkTarget === 'tests') {
                            navigateTo('tests');
                          } else if (n.linkTarget === 'results') {
                            navigateTo('results');
                          }
                        }
                        setNotifOpen(false);
        ...
        <div ref={profileRef} className="relative pl-1 border-l border-slate-200">
          ...
          >
            ...
            />
            <span className="hidden xl:inline-block text-xs font-semibold text-slate-800 truncate max-w-[120px]">
              {user.fullName}
            </span>
          ...
          {profileOpen && (
            <div className="absolute right-0 top-12 w-56 bg-white rounded-2xl shadow-2xl border border-slate-200 py-2 z-50 animate-in fade-in zoom-in-95">
              <div className="px-4 py-3 border-b border-slate-100">
                ...
                <div className="mt-1.5 inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-100 text-blue-700">
