...
import { GraduationCap, Eye, EyeOff, Lock, Mail, AlertCircle, ArrowRight } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Email yoki parol noto‘g‘ri.';
      setError(message);
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-4xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden grid grid-cols-1 md:grid-cols-2 border border-slate-200/20">
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                ...
                <button
                  type="button"
                  onClick={() => setError('Parolni tiklash uchun platforma administratoriga murojaat qiling.')}
          ...
          <div className="mt-8 pt-6 border-t border-slate-100">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className="py-2 px-1 rounded-xl bg-slate-100 hover:bg-blue-50 text-blue-700 text-[11px] font-bold transition-colors"
              >
                Talaba
              </button>
              <button
                onClick={() => handleDemoLogin('teacher')}
                className="py-2 px-1 rounded-xl bg-slate-100 hover:bg-purple-50 text-purple-700 text-[11px] font-bold transition-colors"
              >
                O‘qituvchi
              </button>
              <button
                onClick={() => handleDemoLogin('admin')}
                className="py-2 px-1 rounded-xl bg-slate-100 hover:bg-rose-50 text-rose-700 text-[11px] font-bold transition-colors"
              >
                Admin
              </button>
            </div>
        ...
        <div className="hidden md:flex bg-gradient-to-br from-blue-600 via-indigo-600 to-slate-900 p-12 text-white flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-4">
            <span className="px-3 py-1 rounded-full bg-white/10 backdrop-blur-md text-[10px] font-bold uppercase tracking-wider text-blue-200">
              O‘quv jarayoni 2026
            </span>
            <h3 className="text-2xl font-bold tracking-tight leading-snug">
              Nazariya, Taqdimot va Videolar Bir Joyda!
            </h3>
            <p className="text-xs text-blue-100 leading-relaxed opacity-90">
              Universitet va o‘quv markazi talabalari uchun yaratilgan zamonaviy elektron ta’lim tizimi. Darslarni o‘rganing, test topshiring va o‘quv statistikangizni kuzatib boring.
            </p>
          </div>

          <div className="relative z-10 p-4 rounded-2xl bg-white/10 border border-white/10 backdrop-blur-md space-y-2">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold">
                ✓
              </div>
              <div>
                <p className="text-xs font-bold text-white">Anvar Axadjonov (SE-301)</p>
                <p className="text-[10px] text-blue-200">O‘zlashtirish darajasi: 68%</p>
              </div>
            </div>
          </div>
        </div>
