...
const WeeklyActivityChart = React.lazy(() =>
  import('../components/dashboard/WeeklyActivityChart').then((module) => ({ default: module.WeeklyActivityChart }))
);

const ChartSkeleton: React.FC = () => (
  <div className="p-6 rounded-2xl bg-white border border-slate-100 shadow-xs h-[316px] flex items-center justify-center">
    <div className="flex items-center gap-3 text-xs font-semibold text-slate-400">
      <Loader2 className="w-4 h-4 animate-spin text-indigo-600" />
      Faollik grafigi yuklanmoqda...
    </div>
  </div>
);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <StatCard title="Jami Kurslar" value={courses.length} subtitle="Faol talaba" icon={BookOpen} iconBgColor="bg-blue-50" iconTextColor="text-blue-600" />
        <StatCard title="Tugallangan" value={user.completedLessonsCount} subtitle={`Jami ${user.totalLessonsCount} darsdan`} icon={CheckCircle2} iconBgColor="bg-emerald-50" iconTextColor="text-emerald-600" />
        <StatCard title="O‘zlashtirish" value={`${user.overallProgress}%`} subtitle="Barcha fanlar" icon={TrendingUp} iconBgColor="bg-indigo-50" iconTextColor="text-indigo-600" />
        <StatCard title="Ishlangan Testlar" value={testResults.length} subtitle={`Mavjud ${testList.length} ta test`} icon={FileQuestion} iconBgColor="bg-amber-50" iconTextColor="text-amber-600" />
        <StatCard title="O‘rtacha Natija" value={`${user.averageScore}%`} subtitle="Test bali" icon={Award} iconBgColor="bg-purple-50" iconTextColor="text-purple-600" />
        <StatCard title="Sarflangan Vaqt" value={`${Math.floor(user.totalStudyMinutes / 60)}s`} subtitle="O‘quv vaqti" icon={Clock} iconBgColor="bg-rose-50" iconTextColor="text-rose-600" />
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => navigateTo('courses')} className="text-xs font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {activeCourses.map((course) => <ActiveCourseCard key={course.id} course={course} />)}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-rose-100 text-rose-700">{availableTests.length} ta mavjud</span>
          </div>
          ...
          <div className="space-y-3">
            {nextTests.length === 0 ? (
              ...
              <div key={test.id} className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 space-y-1 text-xs">
                ...
                <button onClick={() => navigateTo('test_taking', { testId: test.id })} className="mt-2 text-[11px] font-semibold text-blue-600 hover:underline flex items-center gap-1">
